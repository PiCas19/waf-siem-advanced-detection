...
func (d *Detector) checkValue(r *http.Request, param, value string) *Threat {
	if detected, desc := d.xss.Detect(value); detected {
		return &Threat{Type: "XSS", Description: desc, Severity: "HIGH", Payload: value}
	}
	if detected, desc := d.sqli.Detect(value); detected {
		return &Threat{Type: "SQL_INJECTION", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.nosql.Detect(value); detected {
		return &Threat{Type: "NOSQL_INJECTION", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.lfi.Detect(value); detected {
		return &Threat{Type: "LFI", Description: desc, Severity: "HIGH", Payload: value}
	}
	if detected, desc := d.pathTraversal.Detect(value); detected {
		return &Threat{Type: "PATH_TRAVERSAL", Description: desc, Severity: "HIGH", Payload: value}
	}
	if detected, desc := d.rfi.Detect(value); detected {
		return &Threat{Type: "RFI", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.ssrf.Detect(value); detected {
		return &Threat{Type: "SSRF", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.cmdInj.Detect(value); detected {
		return &Threat{Type: "COMMAND_INJECTION", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.xxe.Detect(value); detected {
		return &Threat{Type: "XXE", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.ldap.Detect(value); detected {
		return &Threat{Type: "LDAP_INJECTION", Description: desc, Severity: "HIGH", Payload: value}
	}
	if detected, desc := d.ssti.Detect(value); detected {
		return &Threat{Type: "SSTI", Description: desc, Severity: "CRITICAL", Payload: value}
	}
	if detected, desc := d.respSplit.Detect(value); detected {
		return &Threat{Type: "HTTP_RESPONSE_SPLITTING", Description: desc, Severity: "HIGH", Payload: value}
	}
	if detected, desc := d.protoPollution.Detect(value); detected {
		return &Threat{Type: "PROTOTYPE_POLLUTION", Description: desc, Severity: "HIGH", Payload: value}
	}
	return nil
}