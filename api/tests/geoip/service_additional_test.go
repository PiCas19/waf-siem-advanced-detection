...
// Test EnrichIPFromService with HTTP errors
func TestService_EnrichIPFromService_HTTPErrors(t *testing.T) {
	service, err := geoip.NewService()
	require.NoError(t, err)
	...
	t.Run("HTTP 404 error", func(t *testing.T) {
		...
		// Test with a real public IP (but server will return 404)
	...
	t.Run("Missing country_name field", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
	...
	t.Run("Empty country_name field", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			response := map[string]interface{}{
				"ip":           "8.8.8.8",
